...
function findLongLines {
	...
	if [ "X${DO_EXECUTE}" = "X1" ]; then
		...
		if [ "X$FILE_PATTERN" = "X" ]; then
			find "${FIND_DIR}" -print0 | xargs -I_ awk "${AWK_COMMAND}" _
		else
			find "${FIND_DIR}" -name "${FILE_PATTERN}" -print0 | xargs -I_ awk "${AWK_COMMAND}" _
		fi
	fi
}
...
function processSize {
	...
	if [ "X$FILTER_IN" = "X1" ]; then
		if [ "X$FILTER_OUT" = "X1" ]; then
			ps -a | grep -v -E "grep" | cut -f7 -d' ' | xargs -I_ cat /proc/_/status | grep -i -E "vmsize|pid" | grep -v -E "PPid|Tracer"
		else
			ps -a | grep -v -E "${FILTER_OUT}|grep" | grep Daem | cut -f7 -d' ' | xargs -I_ cat /proc/_/status | grep -i -E "vmsize|pid" | grep -v -E "PPid|Tracer"
		fi

	else
		if [ "X$FILTER_OUT" = "X1" ]; then
			ps -a | grep -v -E "grep" | grep "${FILTER_IN}" | cut -f7 -d' ' | xargs -I_ cat /proc/_/status | grep -i -E "vmsize|pid" | grep -v -E "PPid|Tracer"

		else
			ps -a | grep -v -E "${FILTER_OUT}|grep" | grep "${FILTER_IN}" | cut -f7 -d' ' | xargs -I_ cat /proc/_/status | grep -i -E "vmsize|pid" | grep -v -E "PPid|Tracer"
...
\t\ttracking ${EFRED}upstream${ERS}:\torigin/task/${EFGRN}myTask/xyz${ERS}\n"
	...
	while getopts t:f:ygd option; do
		case "${option}" in
		t) TASK_ARG=${OPTARG} ;;
...
\t\ttracking ${EFRED}upstream${ERS}:\torigin/${EFGRN}${BRANCH_NAME}${ERS}\n"
	...
	while getopts j:b:r:he option; do
		case "${option}" in
		j) JIRA_TEXT=${OPTARG} ;;
		j) REPO_DIR=${OPTARG} ;;
		e) DRY_RUN="" ;;
...
function findTests() {
	...
	set COMP_TEST_DIRS= "${ROOT_DIR}/sub1" \
		"${ROOT_DIR}/sub2" \
		"${ROOT_DIR}/sub3" \
		"${ROOT_DIR}/sub4"
